...
impl Unquote {
    ...
    pub fn quote(input: &str) -> String {
        let mut result = String::from(Self::QUOTE);
        result.extend(input.chars().flat_map(|c| match c {
            Self::ESCAPE | Self::QUOTE => [Self::ESCAPE, c].into_iter().take(2),
    ...
    /// Позиция первой неэкранированной закрывающей кавычки
    fn find_closing_quote(s: &str) -> Option<usize> {
        s.char_indices()
            .scan(false, |escaped, (i, c)| {
                let was_escaped = *escaped;
                *escaped = c == Self::ESCAPE && !was_escaped;
                Some((i, c, was_escaped))
            })
            .find_map(|(i, c, was_escaped)| {
                (c == Self::QUOTE && !was_escaped).then_some(i)
            })
    }

    /// Убрать экранирование: `abc\"def` -> `abc"def`
    fn unescape(s: &str) -> String {
        s.chars()
            .fold((String::new(), false), |(mut acc, escaped), c| {
                if c == Self::ESCAPE && !escaped {
                    (acc, true)
                } else {
                    acc.push(c);
                    (acc, false)
...
impl Parser for Unquote {
    type Dest = String;
    /// Распарсить строку, которую ранее [обернули в кавычки](Unquote::quote)
    // `"abc\"def\\ghi"nice` -> (`abc"def\ghi`, `nice`)
    fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ()> {
        let inner = input.strip_prefix(Self::QUOTE).ok_or(())?;
        let close_pos = Self::find_closing_quote(inner).ok_or(())?;
        let quoted_content = &inner[..close_pos];
        let remaining = &inner[close_pos + Self::QUOTE.len_utf8()..];
        let unescaped = Self::unescape(quoted_content);
...
impl Parser for QuotedTag {
    ...
    /// (упрощённая версия [`Unquote`], в которой вложенные кавычки не предусмотрены)
    fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ()> {
        let inner = input.strip_prefix(Unquote::QUOTE).ok_or(())?;
        let quote_idx = inner.find(Unquote::QUOTE).ok_or(())?;
        if quote_idx == 0 || inner.as_bytes().get(quote_idx - 1) == Some(&(Unquote::ESCAPE as u8)) {
            return Err(());
        }
        let candidate = &inner[..quote_idx];
        if !self.0.parse(candidate)?.0.is_empty() {
            return Err(());
        }
        Ok((&inner[quote_idx + 1..], ()))
    }
}
